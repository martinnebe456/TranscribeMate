...
# 7) Optionally produce ZIP + SHA256 checksum in dist/<os>/.
# ============================================================================
...
Usage:
  ...
  .\scripts\windows\build_v2_release.ps1 [-SkipFrontendBuild] [-AppVersion <version>] [-CodeSignThumbprint <thumbprint>] [-CodeSignTimestampUrl <url>] [-RequireCodeSigning] [-SkipZipCreation] [-Help]
...
Parameters:
    ...
    -SkipZipCreation:
        Skip creating the ZIP release package. The app image will still be created in dist/windows/TranscribeMate.
...
Example:
    ...
    .\scripts\windows\build_v2_release.ps1 -AppVersion "26.02.18.005"
    
    .\scripts\windows\build_v2_release.ps1
    
    .\scripts\windows\build_v2_release.ps1 -SkipZipCreation
...
Then run .\scripts\windows\build_v2_release.ps1 again.
"@
    throw $hint
}
...
Close running TranscribeMate instances and close any Explorer window opened inside dist/windows/TranscribeMate.
If antivirus is scanning files, wait a moment and run .\scripts\windows\build_v2_release.ps1 again.
"@
    throw $hint
}
...
function Validate-BundledBackend([string]$BundledBackendDir) {
    ...
    $required = @(
        ...
        @{ Path = (Join-Path $BundledBackendDir "bootstrap_runtime.sh"); Description = "macOS runtime bootstrap script" },
...
$runtimeBootstrapScript = Join-Path $root "scripts\windows\bootstrap_runtime.ps1"
...
$runtimeBootstrapShellScript = Join-Path $root "scripts\macos\bootstrap_runtime.sh"
if (Test-Path $runtimeBootstrapShellScript) {
    Copy-Item -Path $runtimeBootstrapShellScript -Destination (Join-Path $bundledBackendDir "bootstrap_runtime.sh") -Force
} else {
    throw "[TM] macOS runtime bootstrap script not found: $runtimeBootstrapShellScript"
}
