...
function Write-RuntimeReadyMarker(
    ...
    try {
        ...
        $arch = ([string]([System.Runtime.InteropServices.RuntimeInformation]::OSArchitecture)).Trim().ToLowerInvariant()
        ...
        $payload = [ordered]@{
            ...
            python_relpath = "runtime/python/python.exe"
            ffmpeg_relpath = "assets/ffmpeg.exe"
            ffprobe_relpath = "assets/ffprobe.exe"
...
try {
    ...
    $overrideAppData = [string]$env:TM_APP_DATA_DIR
    if ([string]::IsNullOrWhiteSpace($overrideAppData)) {
        ...
        $appDataDir = [System.IO.Path]::GetFullPath($overrideAppData)
    }
