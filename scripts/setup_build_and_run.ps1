...
function Invoke-ReleaseBuild([string]$RepoRoot) {
    $buildScript = Join-Path $RepoRoot "scripts\build_v2_release.ps1"
...
function Try-LaunchBuiltApp([string]$RepoRoot) {
    $exePath = Join-Path $RepoRoot "dist\TranscribeMate\TranscribeMate.exe"
...
$repoRoot = if (Test-Path (Join-Path $candidateRoot "transcribemate")) { $candidateRoot } else { $scriptDir }
