...
# Fix Windows symlink permission issue for HuggingFace cache
...
I18N: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "mode.hint.conference": "Conference mode: pick a folder and export transcripts to .txt/.md only.",
    ...
    "cs": {
        ...
        "mode.hint.conference": "Režim Konference: vyber složku a ulož přepisy do .txt/.md.",
...
# -----------------------------
# Runtime paths (supports PyInstaller)
# -----------------------------
def app_root() -> Path:
...
# -----------------------------
# GPU / VRAM detection + auto model selection
# -----------------------------
@dataclass
class GpuInfo:
    available: bool
...
# -----------------------------
# Subprocess runner (captured output, stoppable)
# -----------------------------
_YTDLP_PCT = re.compile(r"(\d{1,3}\.\d)%")
_YT_URL_PATTERN = re.compile(
    r"^(https?://)?(www\.)?(youtube\.com|youtu\.be|youtube-nocookie\.com)/.+"
)


def safe_run(cmd: List[str], cwd=None, on_line=None, stop_flag=None, env=None):
    p = subprocess.Popen(
        cmd,
        cwd=cwd,
...
def force_refresh_models(log):
    ...
    log(f"[UPDATE] Removing Whisper cache folder: {wh}\n")
    if wh.exists():
        shutil.rmtree(wh, ignore_errors=True)

    log(f"[UPDATE] Removing HuggingFace cache folder: {hf}\n")
    if hf.exists():
        shutil.rmtree(hf, ignore_errors=True)

    log("[UPDATE] Done. Next run will re-download models.\n")
...
def unique_path(directory: Path, filename: str) -> Path:
    ...
    for idx in range(2, 1000):
        ...
        if not candidate.exists():
            return candidate
    raise RuntimeError(f"Could not create unique path for {filename}")


def split_segments(segments: List, part_seconds: int) -> List[List]:
    if part_seconds <= 0:
        return [segments]
    buckets: Dict[int, List] = {}
    for seg in segments:
        part_idx = int(float(seg.start) // part_seconds)
        buckets.setdefault(part_idx, []).append(seg)
...
def download_single_or_playlist(url: str, workdir: Path, is_playlist: bool, quality: str,
    ...
    def on_line(line: str):
        ...
        if m:
            try:
                set_step_progress(float(m.group(1)))
            except Exception:
                pass
...
def faster_whisper_transcribe(media_path: Path, out_dir: Path, model: str, prefer_gpu: bool,
    ...
    """Run transcription and return segments + intermediate files."""
    ...
    log(f"[INFO] Transcription complete: {srt_path}\n")
    log(f"[INFO] Transcript saved: {txt_path}\n")
...
def translate_srt(inp_srt: Path, out_srt: Path, model_name: str, prefer_gpu: bool, batch_size: int,
    ...
    try:
        ...
        log("[WARN] Safetensors nedostupný, používám standardní pytorch formát.\n")
        mdl = AutoModelForSeq2SeqLM.from_pretrained(
            model_name,
            torch_dtype="auto",
        ).to(device)
...
def hard_subtitles(video_path: Path, srt_path: Path, out_mp4: Path,
    ...
    # Build style string for subtitles filter
    primary_color = hex_to_ass_color(color)
    border_color = hex_to_ass_color(outline_color)

    style = f"FontName={font},FontSize={size},PrimaryColour={primary_color},OutlineColour={border_color},Outline={outline_width},BorderStyle=1"
